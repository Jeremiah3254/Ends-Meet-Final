...
public class L0PeasantAbiltiesScript : MonoBehaviour
{
    ...
    void RightHook(int index) {
        ...
        if ((currentEnemyReference != null) && (Vector3.Distance(currentEnemyReference.transform.position,StateNameController.playerCharacter.transform.position) <= 2)) {// range from ability + 1.1;
            currentEnemyReference.GetComponent<StatusManager>().health = currentEnemyReference.GetComponent<StatusManager>().health - ((StateNameController.playerCharacter.GetComponent<PlayerMovement>().characterDamage+StateNameController.damageBoost)*1.25f);
        }
